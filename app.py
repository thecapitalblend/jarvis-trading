...
# ----------------------------
# COMBINED SIGNAL (Technical + News)
# ----------------------------
combined_score = score + (1 if news_score > 1 else (-1 if news_score < -1 else 0))
combined_verdict = "🟢 Overall Bullish" if combined_score > 0 else (
    "🔴 Overall Bearish" if combined_score < 0 else "🟡 Overall Neutral")
st.markdown("---")
st.subheader("🎯 Combined Signal (Technical + News)")
st.write(f"**{combined_verdict}** (combined score: {combined_score})")
st.caption("Astrology module abhi jodna baaki hai — ye score sirf technical + news par based hai.")
